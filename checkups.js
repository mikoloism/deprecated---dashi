...
const is = {
	...
	has = {
		...
		none: (subject) =>
			has.usable(subject) &&
			!has._null(subject) &&
			(is.str(subject) || is.array(subject)
				? subject.length > 0
				: true) &&
			subject !== '' &&
			subject !== '' &&
	...
	regex = {
		...
		signF: (subject) =>
			/^\-{0,1}[0-9]{1,}(\.{0,1}[0-9]{1,}|[0-9]{0,})$/.test(subject),
		// email: (subject) => (/^[a-z]{1,}([0-9\.]{0,}[a-z]{1,}|[a-z0-9]{2,})\.{0,0}@[a-z]{3,}\.[a-z]{2,3}$/i.test(subject))
	},
	...
	dock = {
		...
		removeClass: (subject, className) => {
			...
			if (has.str(className)) {
				if (has.array(c))
					c.forEach((i) => i.classList.remove(className));
		...
		toggle: (subject, className1, className2) => {
			...
			if (has.str(className1)) {
				if (has.usable(className2) && has.str(className2)) {
					...
					if (has.array(c))
						c.forEach((i) => i.classList.toggle(className1));
